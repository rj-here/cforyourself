...
char expression[MAX_LEN];
sprintf(expression, "%d ^ %d = %d\n", base, power, poweredBase); //formatting the expression, to add to history
addToHistory(expression, hist); //Adding to history
...
int divide(int num1, int num2) {
if (num2 == 0) {
    printf("Error: Division by zero is not allowed.\nThis will not be added to history!\n");
...
if (denominator1 == numerator2) { //if denominator of first is numerator of second
    ...
    printf("The fraction is: %.6f (up to 6 decimal places)\n", poweredFraction); //Printing the fraction
    char expression[MAX_LEN];
    sprintf(expression, "(%d / %d) ^ (%d / %d) = %.6f", numerator1, denominator1, numerator2, denominator2, poweredFraction); //formatting the expression, to add to history
    addToHistory(expression, hist); //Adding to history
}
